# iMessage Number Checker API — Bulk Verification | NumberChecker

The **iMessage Number Checker API** accepts a text file of phone numbers, creates an asynchronous bulk task, and exports a result file. This repository contains a runnable integration in eight languages.

- **Product page:** https://numberchecker.ai/products/imessage-checker
- **API documentation:** https://docs.numberchecker.ai/imessage-bulk-number-checker
- **API base URL:** `https://api.numberchecker.ai`
- **Authentication:** `X-API-Key`
- **Get an API key:** https://platform.numberchecker.ai

> [!NOTE]
> This repository covers the standalone product code `imessage` only. Other product codes are outside its scope.

## Product facts

| API fact | Value |
|---|---|
| Product code | `imessage` |
...
1. Submit the input file to `POST /v1/tasks` with `task_type=imessage`.
...
Only `imessage`. The task type is intentionally fixed to that product code.
...
- **Product:** https://numberchecker.ai/products/imessage-checker
- **Documentation:** https://docs.numberchecker.ai/imessage-bulk-number-checker
